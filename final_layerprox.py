"""
LayerProx: layer-wise proximal regularization for federated learning.

Trains FedAvg / FedProx / LayerProx (cosine- and magnitude-gated variants) on
CIFAR-10 with a CIFAR-style ResNet-20 under a Dirichlet non-IID partition.

Run:
    python layerprox.py

Each run writes its accuracy history to ./results as JSON, so a long sweep can
be resumed or inspected without re-running completed configurations. Setting
debug=True enables per-group diagnostic prints for the first two rounds.
"""
...
def default_config():
    return dict(
        ...
        n_clients=10,
        participation=1.0,
        alpha=0.1,              # Dirichlet concentration over labels
        rounds=25,
        local_epochs=5,
        batch_size=64,
        lr=0.01,
...
def group_key(name):
    # Strip the torch.compile prefix so grouping matches the eager model.
...
# --------------------------------------------------------------------------- #
# LayerProx: per-layer mu from a local drift signal
# --------------------------------------------------------------------------- #
@torch.no_grad()
def _flatten_group(named_tensors, names):
    return torch.cat([named_tensors[n].flatten() for n in names])

# Return a {group: mu} map for one client in the current round.
def compute_mu_per_group(model, global_state, prev_state, loader, cfg, device,
    ...
    # First round has no previous global update; fall back to a uniform warm-up.
    if prev_state is None:
        mu_map = {g: cfg["mu_base"] * 0.5 for g in groups}
        if debug:
            print(f"\n[debug r={_round} warmup] mu_base={cfg['mu_base']} sigmoid_alpha={cfg['sigmoid_alpha']}")
            print(f"  prev_state is None -> mu = mu_base/2 = {cfg['mu_base'] * 0.5:.5f} (uniform)")
        return mu_map

    # Single-batch gradient at the global weights w^t.
    ...
    # Delta w_l = w^{t-1} - w^t points along the previous global gradient.
    ...
    for g, names in groups.items():
        ...
        if cfg.get("use_magnitude", False):
            # Magnitude projection: per-group gradient energy.
            delta[g] = gv_norm
        else:
            # Direction projection: drift as one minus cosine alignment.
    ...
    if debug:
        print(f"\n[debug r={_round} cid={_cid}] mu_base={cfg['mu_base']} sigmoid_alpha={a}")
        ...
        print(f"  mu range: [{min(mu_map.values()):.5f}, {max(mu_map.values()):.5f}]"
            ...
            with torch.amp.autocast(device_type='cuda', dtype=torch.float16):
                ...
                if cfg["method"] != "fedavg" and prox_params:
                    ...
                    if debug and not printed_debug:
                        ...
                        print(f"  prox term = {prox_val:.4f}  (0.5 * sum mu_l ||p - anchor||^2)")
...
def aggregate(client_states, weights):
    ...
    for k in agg:
        if agg[k].dtype.is_floating_point:
            ...
            # Integer buffers (e.g. BN num_batches_tracked) are not averaged.
...
def run_fl(cfg, data_root="./data"):
    ...
    print(f"\n[device={device}] {cfg['method']:9s} mu={cfg['mu_base']} "
          f"a_sig={cfg.get('sigmoid_alpha', '-')} dir_alpha={cfg['alpha']} seed={cfg['seed']}")
    ...
    for r in range(cfg["rounds"]):
        ...
        # Move the server state to the GPU once per round and reuse it.
        global_state_gpu = {k: v.to(device, non_blocking=True) for k, v in global_state.items()}
        prev_state_gpu = ({k: v.to(device, non_blocking=True) for k, v in prev_state.items()}
...
if __name__ == "__main__":
    base = default_config()
    base.update(n_clients=10, rounds=100, local_epochs=5, eval_every=5, debug=False)
    ...
    for alpha in [0.1]:
        for seed in [0, 1]:
            ...
            # FedProx at the tuned scalar optimum.
            configs.append(dict(base, method="fedprox", mu_base=0.01, alpha=alpha, seed=seed))

            # LayerProx, cosine (direction) projection.
            configs.append(dict(base, method="layerprox", mu_base=0.01, sigmoid_alpha=5.0,
                                use_magnitude=False, alpha=alpha, seed=seed))

            # LayerProx, magnitude projection. The gradient norm is large, so a
            # smaller sigmoid_alpha keeps the gate out of permanent saturation.
